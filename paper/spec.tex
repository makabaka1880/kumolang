...
s & ::= & \text{\texttt{skip}} & \text{(Stasis)} \\
  ...
  & \mid  & y \leftarrow \text{\texttt{prep}}(x) & \text{(Monadic pure: lift mixture $x$ to container $y$)} \\
  & \mid  & y \leftarrow \text{\texttt{pour}}(y, z) & \text{(Monadic join: merge container $z$ into container $y$)} \\
  & \mid  & z \leftarrow \text{\texttt{pour-out}}(y, x) & \text{(Type lifting: lift container $y$ into domain $x$)} \\
\end{array}
\]
