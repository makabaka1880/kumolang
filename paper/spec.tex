...
\subsection{Semantic DAG Generation}
Because of the linear nature of resources in KumoLang, the dependency graph of any KumoLang program is guaranteed to be a directed acyclic graph (DAG). Each node in the DAG corresponds to a variable binding event, and each directed edge represents a resource flow or dependency induced by the operational semantics. For example, if variable $y$ is derived from variable $x$ through a \texttt{prep} operation, there will be a directed edge from the node representing $x$ to the node representing $y$. Similarly, if variable $z$ is derived from variables $y_1$ and $y_2$ through a \texttt{pour} operation, there will be directed edges from both $y_1$ and $y_2$ to $z$. The absence of cycles in this graph is guaranteed by the semantics, as resources cannot be reused or reintroduced once consumed. This structural property allows the Kumo Verification Engine to efficiently extract and represent the provenance of the synthesized pastry artifact as a DAG, which can then be used for further analysis or verification.
...
The Kumo Verification Engine extracts this graph automatically from the operational trace. Provenance queries may subsequently be answered through graph traversal, while culinary genealogy analysis may be performed through subgraph matching and graph-isomorphism techniques. 
